...
# Downloading the dataset, splitting into train/test images,
...
class DIV2K:
    ...
    def dataset(self, batch_size=16, repeat_count=None, random_transform=True):
        ...
        if random_transform:
            ds = ds.map(lambda lr, hr: CropImage(lr, hr, scale=self.scale), num_parallel_calls=AUTOTUNE)
            ds = ds.map(RotateImage, num_parallel_calls=AUTOTUNE)
            ds = ds.map(FlipImage, num_parallel_calls=AUTOTUNE)
