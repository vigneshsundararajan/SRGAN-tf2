...
# Defining the general training class which 
# then further defines the training functions
class Trainer:
    def __init__(self,
                 model,
                 loss,
    ...
    def train(self, train_dataset, valid_dataset, steps, evaluate_every=1000, save_best_only=False):
        ...
        for lr, hr in train_dataset.take(steps - ckpt.step.numpy()):
            ...
            if step % evaluate_every == 0:
                ...
                # Printing out the loss and PSNR values
                print(f'{step}/{steps}: loss = {loss_value.numpy():.3f}, PSNR = {psnr_value.numpy():3f} ({duration:.2f}s)')

                if save_best_only and psnr_value <= ckpt.psnr:
                    self.now = time.perf_counter()
                    # Don't save checkpoint if there is no 
                    # improvement in PSNR
                    continue
...
# Defining SRGAN specific Training Functions
class SrganGeneratorTrainer(Trainer):
    ...
    # Can reduce the number of steps to 
    # reduce computational time. 100,000 is okay
    def train(self, train_dataset, valid_dataset, steps = 1000000, evaluate_every = 1000, save_best_only = True):
        super().train(train_dataset, valid_dataset,
                      steps, 
                      evaluate_every,
                       save_best_only)
...
class SrganTrainer:
    def __init__(self,
                 ...
                 discriminator,
                 content_loss='VGG54',
                 # Using Adaptive Learning to vary LR as 1e-4 up to first
                 # 100,000 steps, and then changing LR to 1e-5 using
                 # Piecewise Decay
    ...
    def train(self, train_dataset, steps=200000):
        ...
        for lr, hr in train_dataset.take(steps):
            ...
            if step % 50 == 0:
                # Printing out Perceptual and Discriminator Losses
                # every 50 steps
